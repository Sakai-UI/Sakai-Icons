...
const defaultConfig = require("../sakai-icons.json");
...
function updateConfig(config) {
  ...
  for (let key in config) {
    log.info(`sakai-icons.${key} changed, updating to ${config[key]}`);
    const updateHandler = updateThemeJSONHandlers[key];
    if (updateHandler) {
      vscode.workspace.getConfiguration("sakai-icons").update(key, config[key], true);
